...
multi=[0.0001 1000];se=[2,1].*size(y0);sen=ones(se);uzi=length(y0);
%
for lol=1:1:uzi
   y=y0;
   lgd1=[multi(1)*y(lol) y(lol)];
   lgd2=[y(lol) multi(2)*y(lol)];
   [a,b]=findbound(lol,y0,lgd1);
   sen(1,lol)=(a+b)/2;
   [c,d]=findbound(lol,y0,lgd2);
   sen(2,lol)=(c+d)/2;
end 
...
function ssnt=runode(y,N)
YJ=[];nbatch=1;T=4000;h=0.005;nstep=T/h;
for ii=1:N
