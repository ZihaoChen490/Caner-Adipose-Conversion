...
p0_pM=y(171);p0_pm1=y(172);p0_pm2=y(173);p0_pm3=y(174);P530=y(175);rkip0_rl=y(176);let0_ls=y(177);let0_let=y(178);let0_llin =y(179);let0_lb=y(180);
...
df(4,:)=g_s.*Hs(snail,snail0_ss,lambda_ss,n_ss).*TGF-k_s.*snail.*Hs(miR34,miR340_m3s,lambda_m3s,n_m3s).*Hs(let,let0_ls,lambda_ls,n_ls).*Hs(rkip,rkip0_rs,lambda_rs,n_rs)./8;
...
df(9,:)=g_rk.*9.*Hs(bach,bach0_br,lambda_br,n_br)-k_rk.*rkip.*Hs(snail,snail0_sr,lambda_sr,n_sr)./5;
...
df(11,:)=g_lin.*Hs(MEK,MEK0_Ml,lambda_Ml,n_Ml).*Hs(oct4,oct40_ol,lambda_ol,n_ol).*Hs(rkip,rkip0_rli,lambda_rli,n_rli)-k_lin.*lin.*Hs(let,let0_llin,lambda_llin,n_llin).*Hs(lin,lin0_lin,lambda_lin,n_lin).*Hs(miR200,miR2000_m2li,lambda_m2li,n_m2li);
df(12,:)=g_b.*10.*Hs(bach,bach0_bb,lambda_bb,n_bb)-k_b.*bach.*Hs(let,let0_lb,lambda_lb,n_lb)./5;
...
%{
y=[1.9	1.4	 1	1	0.01	0.16	0.04  0.04	3	144,... %1-10
    0.032	57	5 100	40	4	80	4	2	4,...           %11-20
    2	2    4	2	2	2	100	100	 1000 	1500,...        %21-30
    1500 1540 2000 3000	1000 4.3 200 0.8 1.5  0.1,...       %31-40
    6000 6 2 2000 2 200 200 200 800 200,...                 %41-50
    4 20 20 6 10 10 10 5 20 5,...                           %51-60
    20 25 0.09 0.1 0.1 0.01 0.04 0.04 0.04 0.1,...          %61-70
    0.01 0.1 0.08 0.01 4 2 3 3 0.1 2,...                    %71-80
    4 8 2 5 0.4 6 5 10 2 15,...                             %81-90
    4 15 4 2 4 4 15 4 0.2 20,...                            %91-100
    2 10 2 2 10 2 2 2 4 4,...                               %101-110
    4 8 1 2 2 1 2 2 2 2,...                                 %111-120
    2 2 2 2 2 2 2 4 3 4,...                                 %121-130
    2 2 2 2 4 2 4 2 2 2,...                                 %131-140
    2 2 2 2 2 2 2 2 2 2,...                                 %141-150
    800 400 400 100 800 200 800 1000 400 1000,...           %151-160
    200 200 800 20 400 200 200 200 200 400,...              %161-170
    800 400 400 400 20 200 200 800 20 400,...               %171-180
    200 400 400];                                           %181-190
%}
